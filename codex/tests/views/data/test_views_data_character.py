...
class TestCharacterCRUDViews(TestCase):
    ...
    def test_anonymous_user_cannot_update_characters(self) -> None:
        ...
        test_data = {"classes": "Paladin (Vengeance) 10", "gold": 42.0}
        response = self.client.patch(
            reverse("character-detail", kwargs={"pk": "2"}), json.dumps(test_data), content_type="application/json"
        )
        self.assertEqual(response.status_code, HTTP_403_FORBIDDEN)
    ...
    def test_user_can_update_character(self) -> None:
        ...
        self.assertEqual("Meela", response.data["name"])
