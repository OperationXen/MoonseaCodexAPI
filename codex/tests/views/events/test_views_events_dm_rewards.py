...
class TestDMRewardCRUDViews(TestCase):
    ...
    def test_reward_is_flagged_editable(self) -> None:
        """Check that a dm reward is flagged as editable"""
        self.client.login(username="testuser1", password="testpassword")
        reward = DMReward.objects.get(pk=1)

        response = self.client.get(reverse("dm_reward-detail", kwargs={"pk": reward.uuid}))
        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertTrue(response.data["editable"])

    def test_reward_is_not_flagged_editable(self) -> None:
        """Check that a dm reward is flagged as editable"""
