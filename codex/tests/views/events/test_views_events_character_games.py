...
class TestCharacterGamesCRUDViews(TestCase):
    ...
    def test_user_can_add_own_character_to_game(self) -> None:
        ...
        response = self.client.patch(reverse('game-detail', kwargs={'uuid': game.uuid}), test_data, content_type="application/json")
        self.assertEqual(response.status_code, HTTP_200_OK)
    ...
    def test_user_can_remove_own_character_from_game(self) -> None:
        ...
        response = self.client.delete(reverse('game-detail', kwargs={'uuid': game.uuid}), test_data, content_type="application/json")
        self.assertEqual(response.status_code, HTTP_200_OK)
        self.assertIn('message', response.data)
        self.assertNotIn(game, list(character.games.all()))
