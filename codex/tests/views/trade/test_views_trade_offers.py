...
class TestTradeAdvertViews(TestCase):
    ...
    def test_create_valid_offer(self) -> None:
        ...
        test_data = {
            ...
            "description": "Lantern of revealing for Winged boots",
        }
    ...
    def test_cant_create_offer_for_item_owned_by_same_character(self) -> None:
        ...
        test_data = {
            "item_uuid": item.uuid,
            "advert_uuid": advert.uuid,
            "description": "Both items owned by same character",
        }
        response = self.client.post(reverse("offer"), test_data)
        self.assertEqual(response.status_code, HTTP_400_BAD_REQUEST)
    ...
    def test_update_valid_offer(self) -> None:
        ...
        response = self.client.patch(
            reverse("offer", kwargs={"uuid": offer.uuid}), json.dumps(test_data), content_type="application/json"
        )
        self.assertEqual(response.status_code, HTTP_200_OK)
        offer.refresh_from_db()
